...
#CHECK IF FUNCTIONS WORK, ADD TRAIN VARIABLE TO ABSOLUTE MAGNITUDE TO CHOOSE Z, COMMENT LOOPS, DOCUMENT CLASSIFICATION FUNCTION
...
#IMPORTS
    ...
    #SKLEARN IS A MACHINE LEARNING LIBRARY IN PYTHON THAT WE USE FOR PRINCIPAL COMPONENT ANALYSIS, SYNTHETIC MINORITY OVERSAMPLING TECHNIQUE, RANDOM FORESTS, AND KFOLDING METHODS.
    ...
    #VARNAME IS A DICTIONARY CONTAINING ALL OF THE VARIABLE NAMES FOR OUR PARAMETERS IN PYMC3
        #Tan(Plateau Angle) IS THE VARIABLE NAME USED FOR DATA EXTRACTED DURING THE SUMMER. ANY NEW BATCHES POST AUGUST 2019 MUST USE Arctan(Plateau Slope) AS A VARNAME. I AM NOT CHANGING IT HERE FOR MY CONVENIENCE
...
def Func(t, A, B, gamma, t_0, tau_rise, tau_fall):
    '''
    Calculate the flux given amplitude, plateau slope, plateau duration, start time, rise time, and fall time using numpy.
...
def produce_lc(file, file_npz rand_num):
    '''
    Make a 2-D list containing a random number of light curves created using the parameters from the npz file with shape (number of filters, rand_num).
    ...
    z_index = np.where(np.transpose(new_ps1z)[0] == file_num)[0][0]
...
def absolute_magnitude(file, fltr, norm = True):
    ...
    fltr: string
        One of the elements from lst_filter (g, r, i, z).
    norm : boolean
        If True, it will randomly choose an absolute magnitude with the mean and standard deviation equal to the absolute magnitude and apparent magnitude standard deviation. This randomness is used for producing copies of light curves to make sure not all the copies have the same absolute magnitude. If False, it will choose the actual absolute magnitude.
...
#EVERYTHING FOR THE THIRD CELL WAS PICK AND CHOOSE WHICH TRANSIENTS I WANTED TO USE IN MY SAMPLE. IF YOU HAVE ALL OF THE FILES AND PATHS FOR THE LIGHT CURVES AND THEIR MCMC PARAMETERS, YOU CAN LOOP THROUGH USING THOSE DIRECTLY. FOR REFERENCE:
...
#CREATING COPIES OF THE CLASSIFICATION NUMBERS IN INDEX ORDER. PRINTING OUT THE SHAPES HELPS TO MAKE SURE EVERYTHING IS LOOPING CORRECTLY.
    #[0, 1] --> [0, 0, 0, 0, 1, 1, 1, 1]
...
#CREATING COPIES OF THE LIGHT CURVES IN INDEX ORDER. I DID NOT EXPLICITLY USE THE NPZ_FILE VARIABLE BECAUSE I JUST LOOPED USING IT'S ID NUMBER. YOU WILL NEED TO MAKE A "LST_NPZ" LIST WITH ALL THE NPZ FILES IN INDEX ORDER.
...
#FOR EACH LIGHT CURVE, CONCATENATE ALL THE PRINCIPAL COMPONENTS FOR EACH FILTER INTO ONE ARRAY AND APPEND THE ABSOLUTE VALUES FROM EACH FILTER.
...
def pca_smote_rf (lst_pca, lst_class_id_super, size, n_est, depth = None, max_feat = None):
    '''
    Make a random forest classifier using synthetic minority oversampling technique and kfolding. A lot of the documentation is taken directly from SciKit Learn page and should be referenced for further questions.
    ...
    clf : RandomForestClassifier
        A random forest classifier trained from the classified transients. It will print which fold it is on and a confusion matrix to assess how well the forest performed.
...
#RECREATE LIGHT CURVES FOR THE UNCLASSIFIED TRANSIENTS USING THEIR MCMC PARAMETERS. WILL NEED TO MAKE A LIST OF PATHS FOR THE NPZ FILES OF UNCLASSIFIED TRANSIENTS TO LOOP THROUGH CORRECTLY.
