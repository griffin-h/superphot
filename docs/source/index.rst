...
In Hosseinzadeh et al. (2020, submitted), we trained the classifier on a sample of 557 spectroscopically classified supernovae from the `Pan-STARRS1 <https://panstarrs.stsci.edu>`_ Medium Deep Survey, and then applied it to an additional 2315 transients with host galaxy redshifts. This trained classifier is included in the package, but users will most likely want to train on their own survey data for better performance.
