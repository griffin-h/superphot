...
def mean_lc(file_num, rand_num):
    ...
    z = float(s2c(file_num)[6][1])
    A_v = float(s2c(file_num)[5][1]) * 3.1
    lst = []
    for i in varname:
        lst.append(file[i])
    lst = np.array(lst)
...
#light curve numbers with copies of each object
...
#should this be in log space or no...
...
slsne_num = np.transpose(lst_coswo[2])[0]
...
clf = pca_smote_rf(lst_pca, lst_class_id_super, 0.33, 100, None, None)
...
lst_photo_lc = []
for i in lst_PS1_final:
    lst_photo_lc.append(mean_lc(i, 100))
